...
// 높이 서열: 킥 0.12(바닥) < 플로어 탐 0.26 ≈ 스네어 0.28 < 하이 햇 0.32
...
// 라이드 Y=-0.34 (기본 -0.41에서 이동) — 미들 탐↔플로어 탐 전환 스틱 궤적이
// 라이드를 스치는 문제 해소 (2점 IK 충돌 스윕으로 실측 검증)
// 하이 햇·크래쉬·플로어 탐·라이드는 J2 고정 정책(아래 _IK_LIMITS 참고) 도입 시
// 실측 검증을 거쳐 몸통 쪽으로 소폭 이동 — 하이 햇은 X·Z도 함께 조정해
// (0.48,0.40,0.37)→(0.52,0.32,0.32)로, J1이 다른 왼팔 드럼과 크게 벌어지던
// 문제(±0.85 vs 나머지 -0.79~-1.07)를 절반 수준으로 완화
let drumKit = [
  // ─ L팔 ─────────────────────────────────────────────────────────
  { id:'d0', name:'하이 햇',     type:'hihat', arm:'L', pos:{x:0.52, y: 0.32, z:0.32} },
  { id:'d1', name:'크래쉬 심벌', type:'crash', arm:'L', pos:{x:0.69, y: 0.24, z:0.50} },
  ...
  { id:'d7', name:'라이드 심벌', type:'ride',  arm:'R', pos:{x:0.65, y:-0.34, z:0.48} },
];
let nextDrumId = 8;

// 기본값 스냅샷 (초기화 버튼용)
const DEFAULT_DRUM_KIT = drumKit.map(d => ({...d, pos: {...d.pos}}));
const _DK_STORE = 'openarmx_drum_kit_v10';
...
// "날개를 폈다 접는" 관성이 큰 동작으로 보였다. 실측 결과 J1·J3·J6이
// 이 좁은 J2 구간 안에서도 모든 드럼(팁 오차 <15mm)을 문제없이 커버하고,
// J6도 한계(±1.57)까지 못 미쳐(최대 ~1.26) 여유가 있어 J2를 넓게 풀어줄
...
const _IK_LIMITS = {
  L1:[-2.0, 1.8], L2:[-1.05, -0.75], L3:[-2.9, 2.9],
  L4:[0.05, 1.70], L5:[-2.9, 2.9], L6:[-1.57, 1.57],
  R1:[-1.8, 2.0], R2:[0.75, 1.05], R3:[-2.9, 2.9],
